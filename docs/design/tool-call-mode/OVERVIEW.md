...
```json
{
  ...
  "meta": {
    "exit_reason": "completed | assertion_failed | max_steps_reached | max_actions_reached | agent_error | device_error",
    "duration_ms": 4821,
    "actions_taken": 7
  }
}
...
| `meta.exit_reason` | Yes | Why the command terminated. Distinguishes `assertion_failed` from `max_steps_reached` from `agent_error` etc. |
| `meta.duration_ms` | Yes | Wall-clock time for the command in milliseconds. |
| `meta.actions_taken` | Yes | Number of atomic device interactions executed. 0 for session management commands. |
...
| **exit_reason** | The `meta` field explaining why a command terminated: `completed`, `assertion_failed`, `max_steps_reached`, `max_actions_reached`, `agent_error`, or `device_error`. |
