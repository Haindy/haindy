"""Windows keyboard and mouse input handler using pynput."""
...
# On Windows ctrl is the primary action modifier; meta maps to Key.cmd
# (the Windows key) rather than to cmd as on macOS.
...
class WindowsInputHandler:
    """Handle keyboard and mouse input on Windows via pynput.

    Coordinates accepted by this class are in screenshot pixel space.
    On Windows mss reports logical pixels that match pynput's coordinate
    space, so scale factors are typically 1.0 on non-HiDPI displays and
    the DPI scale factor on HiDPI displays.
    """
    ...
    async def press_key(self, key: str) -> None:
        """Press a key or key combination (e.g. 'enter', 'ctrl+c', 'shift+tab')."""
