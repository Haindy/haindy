"""Windows screen capture using mss."""
...
class WindowsScreenCapture:
    """Capture full-screen images on Windows using mss."""
    ...
    def capture(self, label: str = "screenshot") -> tuple[bytes, str]:
        ...
        logger.debug("Capturing Windows screenshot")
        with _mss.mss() as sct:
            mon = sct.monitors[1]
    ...
    def _persist(self, image_bytes: bytes, label: str) -> tuple[bytes, str]:
        ...
        safe_label = str(label or "windows").replace(" ", "_").replace("/", "-")
