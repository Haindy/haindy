...
from haindy.core.types import BugReport, TestCase, TestState
...
def extract_template_data(
        ...
        test_cases_data.append(
            {
                ...
                "priority": (
                    test_case_meta.priority.value if test_case_meta else "medium"
                ),
    ...
    return {
        ...
        "test_completed": (
            test_report.completed_at.isoformat()
            if test_report.completed_at
            else "In Progress"
        ),
        "environment": (
            json.dumps(test_report.environment, indent=2)
