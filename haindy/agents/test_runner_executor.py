...
from haindy.agents.action_agent import ActionAgentStepSession
from haindy.core.types import ActionInstruction, ActionType, TestStep
    ...
    async def execute_action(
        ...
        action_data = {
            ...
            "ai_conversation": {
                "test_runner_interpretation": (
                    action.copy() if isinstance(action, dict) else None
                ),
        ...
        try:
            ...
            action_data["result"] = {
                ...
                "coordinates": (
                    result.coordinates.model_dump() if result.coordinates else None
                ),
                "execution": (
                    result.execution.model_dump() if result.execution else None
                ),
                "ai_analysis": (
                    result.ai_analysis.model_dump() if result.ai_analysis else None
                ),
            ...
            compatibility_result = {
                ...
                "outcome": outcome,
                "confidence": (
                    result.ai_analysis.confidence if result.ai_analysis else 0.0
                ),
                "error": (
