...
from haindy.agents.test_runner_interpreter import (
    StepInterpretationRequest,
    TestRunnerInterpreter,
)
from haindy.core.types import StepResult, TestCase, TestCaseResult, TestStatus, TestStep
from haindy.monitoring.logger import get_logger
from haindy.runtime.execution_replay_service import (
...
class TestRunnerStepProcessor:
                    ...
                    step_session = await runner.action_agent.begin_step_session(
                        step,
                        {
                            **(
                            ...
                            "test_plan_name": (
                                runner._current_test_plan.name
                                if runner._current_test_plan
                                else None
                            ),
                            "test_case_name": (
                                runner._current_test_case.name
                                if runner._current_test_case
                                else None
                            ),
                            "test_case_id": (
                                runner._current_test_case.test_id
                                if runner._current_test_case
                                else None
                            ),
                            "step_number": step.step_number,
                        ...
                        action_results.append(
                            {
                                ...
                                "full_data": (
                                    runner._current_step_actions[-1]
    ...
    async def execution_replay_key(
        ...
        return await runner._replay_service.execution_replay_key(
            ...
            test_case=test_case,
            current_test_plan_name=(
                runner._current_test_plan.name if runner._current_test_plan else ""
            ),
