...
- setup_steps: actions executed BEFORE the main steps to reach the test case's starting state. Do NOT duplicate actions that are part of the main steps. Choose one of three patterns based on the test case preconditions and the state left by the previous test case:
  ...
  3. HARD RESET (clean app state): the test case requires a completely clean app state — no active session, no cached data (e.g., preconditions say "app freshly launched", "user not signed in", or "new unregistered user"). This may require MULTIPLE setup_steps. The first setup_step should use action "Reset the app to a clean state with no active user session". The expected result of that reset step must describe only the clean reset/relaunch itself, not the final ready screen. Do NOT require the reset step alone to prove that the signed-out welcome/login screen is already visible. If the app or context may show splash/loading screens, startup gates, permission prompts, or any transient post-launch UI before it is ready, emit additional setup_steps such as waiting for the intended ready screen, dismissing startup UI, or opening the target entry screen. Do not write technical commands; the executor will handle the mechanism.
