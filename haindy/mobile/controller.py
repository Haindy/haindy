"""High-level mobile controller with coordinate interactions."""
...
class MobileController:
    """High-level controller for Android automation."""

    def __init__(self, preferred_serial: str | None = None) -> None:
        self.settings = get_settings()
        self.logger = get_logger("mobile.controller")
        resolved_serial = (preferred_serial or "").strip() or (
            self.settings.mobile_default_adb_serial.strip() or None
        )
        self.driver = MobileDriver(
            preferred_serial=resolved_serial,
            adb_timeout_seconds=self.settings.mobile_adb_timeout_seconds,
        )
    ...
    async def start(self) -> None:
        ...
        self.logger.info("Mobile controller started")

    async def stop(self) -> None:
        await self.driver.stop()
        self._initialized = False
        self.logger.info("Mobile controller stopped")
    ...
    async def __aenter__(self) -> MobileController:
        await self.start()
        return self
