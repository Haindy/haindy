...
async def test_virtual_input_falls_back_to_xdotool_when_uinput_unavailable(
    ...
    monkeypatch.setattr("haindy.linux.virtual_input._EVDEV_AVAILABLE", False)
    monkeypatch.setattr(
        "haindy.linux.virtual_input.shutil.which",
        lambda name: "/usr/bin/xdotool" if name == "xdotool" else None,
    )
    monkeypatch.setattr(
        "haindy.linux.virtual_input.asyncio.create_subprocess_exec",
...
def test_virtual_input_raises_without_uinput_or_xdotool(
    ...
    monkeypatch.setattr("haindy.linux.virtual_input._EVDEV_AVAILABLE", False)
    monkeypatch.setattr("haindy.linux.virtual_input.shutil.which", lambda name: None)
