"""
Tests for the Enhanced Test Runner Agent (v2).
"""
...
from src.agents.test_runner_v2 import EnhancedTestRunner
...
def mock_action_agent():
    ...
    mock_coordinates = CoordinateResult(
        grid_cell="M23",
        grid_coordinates=(960, 540),
        offset_x=0.5,
        offset_y=0.5,
        confidence=0.95,
        reasoning="Element found at center of screen",
        refined=False,
    ...
    mock_ai_analysis = AIAnalysis(
        success=True,
        confidence=0.9,
        actual_outcome="Button was clicked and page updated",
        matches_expected=True,
        ui_changes=["Page updated", "New content loaded"],
        recommendations=[],
        anomalies=[]
    )
    
    # Create a dummy test step for the result
    dummy_step = TestStep(
        step_number=1,
    ...
    agent.execute_action = AsyncMock(return_value=mock_result)
    return agent
...
@pytest.fixture
def test_runner_v2(mock_browser_driver, mock_action_agent, tmp_path):
    """Create an EnhancedTestRunner instance for testing."""
...
class TestEnhancedTestRunner:
    """Test cases for EnhancedTestRunner."""
    
    @pytest.mark.asyncio
    async def test_execute_test_plan_success(
        self, test_runner_v2, sample_hierarchical_test_plan, tmp_path
    ):
        """Test successful test plan execution with all test cases passing."""
        # Mock AI responses
        test_runner_v2.call_openai = AsyncMock()
        
        # Mock step interpretation to return simple actions
        test_runner_v2.call_openai.side_effect = [
            # For each step interpretation
            ...
            {"content": json.dumps({"actions": [{"type": "click", "target": "user profile icon", "critical": True}]})},
            {"content": json.dumps({"actions": [{"type": "click", "target": "logout button", "critical": True}]})},
            # For outcome verifications
        ...
        # Execute test plan
        report = await test_runner_v2.execute_test_plan(
    ...
    async def test_execute_test_plan_with_failure(
        self, test_runner_v2, sample_hierarchical_test_plan, mock_action_agent
    ):
        ...
        # Execute test plan
        report = await test_runner_v2.execute_test_plan(sample_hierarchical_test_plan)
    ...
    @pytest.mark.asyncio
    async def test_intelligent_step_interpretation(self, test_runner_v2):
        ...
        test_runner_v2.call_openai = AsyncMock(return_value={
            "content": json.dumps({
                "actions": [
                    {
                        "type": "type",
        ...
        actions = await test_runner_v2._interpret_step(
    ...
    @pytest.mark.asyncio
    async def test_bug_report_generation(self, test_runner_v2):
        ...
        test_runner_v2._current_test_plan = TestPlan(
            plan_id=uuid4(),
            name="Test Plan",
            description="Test",
        ...
        # Generate bug report
        bug_report = await test_runner_v2._create_bug_report(
    ...
    @pytest.mark.asyncio
    async def test_living_document_updates(self, test_runner_v2, tmp_path):
        ...
        test_runner_v2.call_openai = AsyncMock()
        test_runner_v2.call_openai.return_value = {
            "content": json.dumps({
        ...
        # Start execution
        test_runner_v2._test_report = TestReport(
        ...
        await test_runner_v2._save_report()
        
        # Verify files exist
        report_dir = Path(test_runner_v2.report_dir)
        json_files = list(report_dir.glob("*.json"))
        md_files = list(report_dir.glob("*.md"))
        ...
        test_runner_v2._test_report.status = TestStatus.COMPLETED
        await test_runner_v2._save_report()
    ...
    @pytest.mark.asyncio
    async def test_failure_classification(self, test_runner_v2):
        ...
        is_blocker = await test_runner_v2._is_blocker_failure(nav_result)
        ...
        is_blocker = await test_runner_v2._is_blocker_failure(element_result)
        assert is_blocker is True
        test_runner_v2.call_openai.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_summary_calculation(self, test_runner_v2):
        ...
        # Calculate summary
        summary = test_runner_v2._calculate_summary()
