...
def test_prepare_template_data_includes_coordinate_context_and_screenshots() -> None:
    ...
    bug = SimpleNamespace(
        ...
        coordinates_used=SimpleNamespace(
            pixel_coordinates=(800, 500), relative_x=0.5, relative_y=0.5
        ),
    ...
    test_report = SimpleNamespace(
        test_cases=[test_case], bugs=[bug], test_plan_name="Plan"
    )
...
def test_generate_report_writes_html_file(tmp_path: Path) -> None:
    ...
    output = reporter.generate_report(
        test_state, execution_history=[], output_path=tmp_path / "report.html"
    )
