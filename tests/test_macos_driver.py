"""Unit tests for MacOSDriver and MacOSInputHandler using stubs."""
...
def _make_stub_input_class() -> type:
    """Return a MacOSInputHandler replacement that records calls and accepts injected controllers."""
...
async def test_press_key_delegates_to_input_handler(
    ...
    driver = _make_driver(monkeypatch, tmp_path)
    await driver.start()
    await driver.press_key("cmd+c")
    assert driver.input_handler is not None
    assert driver.input_handler.keys == ["cmd+c"]  # type: ignore[attr-defined]
...
async def test_get_viewport_size_returns_pixel_dimensions(
    ...
    # StubScreenCapture captures a 4x4 PNG -> pixel dims are 4x4
    assert w == 4
    assert h == 4
...
# ---------------------------------------------------------------------------
# MacOSInputHandler coordinate scaling unit tests
# ---------------------------------------------------------------------------
...
@pytest.mark.asyncio
async def test_input_handler_no_scale_on_non_retina() -> None:
    """On non-Retina (scale=1.0) coordinates pass through unchanged."""
    stub_mouse = StubMouseController()
    stub_keyboard = StubKeyboardController()
...
@pytest.mark.asyncio
async def test_input_handler_press_key_combination() -> None:
    """Pressing 'cmd+c' should press cmd modifier then 'c' then release both."""
...
async def test_input_handler_scroll_direction() -> None:
    ...
    # 120 pixel down -> -1.0 dy tick (scroll down in pynput convention)
