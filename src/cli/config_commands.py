...
async def handle_config_migrate(dotenv_path: Path, dry_run: bool = False) -> int:
    ...
    if dry_run:
        _CONSOLE.print(f"[bold]Dry run:[/bold] reading {dotenv_path} (no changes will be made)")
    else:
...
def _print_migration_result(result: MigrationResult, dotenv_path: Path) -> None:
    ...
    if not result.settings_written and not result.secrets_stored and not result.secrets_skipped:
    ...
    if result.secrets_skipped:
        _CONSOLE.print(f"[dim]Skipped (empty) API keys for:[/dim] {', '.join(result.secrets_skipped)}")

    if result.settings_written:
        label = "Would write" if result.dry_run else "Wrote"
        _CONSOLE.print(f"[green]{label} {len(result.settings_written)} settings to ~/.haindy/settings.json[/green]")
