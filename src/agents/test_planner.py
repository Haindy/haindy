...
- setup_steps: actions executed BEFORE the main steps to reach the test case's starting state. Use for things like navigating to a specific screen, signing in, opening a deep link, or clearing app data. Leave empty if the test case continues from where the previous one left off or starts naturally. Do NOT duplicate actions that are part of the main steps. Look at the test plan description, context notes, and per-case preconditions to determine what setup is needed.
