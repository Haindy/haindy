...
class StateManager:
    ...
    async def get_test_state(self, test_id: UUID) -> TestState | None:
        """
    ...
    def _get_next_step(self, test_state: TestState) -> TestStep | None:
        """Get the next step to execute."""
        if not test_state.test_plan.steps:
            return None

        # Get all step IDs
        [step.step_id for step in test_state.test_plan.steps]
